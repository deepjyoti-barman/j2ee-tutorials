...
/**
 * It is built to delete existing entry / entries from a table of given MySQL database.
 * 
 * @author Deepjyoti Barman
 * @since March 02, 2020
 */
public class DML_DeleteStatement
{
    ...
    static final String MYSQL_QUERY = "DELETE FROM emp "
            + "WHERE emp_id = 3";
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            System.out.println("Trying to delete an existing entry from the given table of `" + DB_NAME + "` database");
            statement = con.createStatement(); 

            /* STEP-5: Execute a query */
            // If DML failed, it will raise an SQLException
            // No of records affected: 0, means no entry is deleted (i.e. invalid SQL selector written)
            int rows_affected = statement.executeUpdate(MYSQL_QUERY);
            System.out.println("Successfully deleted entry from the given table of `" + DB_NAME + "` database");
